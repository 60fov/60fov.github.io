...
function World(world_data) {
  ...
  this.w = world_data.w;
  this.h = world_data.h;
  this.gravity = world_data.gravity || 0.045;
  ...
  this.update = (delta) => {
    if (this.state == "dying") {
      ...
      if (this.state != "hit") {
        ...
        if (this.jumping) {
          if (keys[action_binds.left[0]] > 1) this.vel.x += -0.5 * delta;
          if (keys[action_binds.right[0]] > 1) this.vel.x += 0.5 * delta;
